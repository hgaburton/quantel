...
class Network:
    ...
    def check_minimum(self, wfn):
        ...
        wfn.get_davidson_hessian_index(plev=0)
        # Return True if the solution is a minimum, False otherwise
        return wfn.hess_index[0] == 0 and self.compare_solutions(wfn0, wfn)
    ...
    def is_new_connection(self, its, min1, min2):
        ...
        print((its, min(min1,min2), max(min1,min2)))
    ...
    def read_solutions(self, prefix="./"):
        ...
        for old_tag in sorted(glob.glob(prefix+"*.solution")):
            ...
            state = self.WFN(self.ints, 'cs')
            state.read_from_disk(old_tag)
            if self.check_minimum(state):
                print("  Solution is a minimum. E = ", state.energy)
                newmin, prev, tag = self.is_new_minimum(state)
                if(not newmin):
                    print("  Solution is equivalent to minimum {:d} with tag {:s}".format(prev, tag))
                else:
                    self.save_minimum(state)
            elif self.check_ts(state):
                print("  Solution is a transition state. E = ", state.energy)
                newts, prev, tag = self.is_new_ts(state)
                if(not newts):
                    print("  Solution is equivalent to transition state {:d} with tag {:s}".format(prev, tag))
                else:
                    (Emin1, Imin1), (Emin2, Imin2) = self.connect(state)
                    self.save_ts(state, Imin1, Imin2)
            else:
    ...
    def connect(self, wfnts):
        ...
        lbfgs = LBFGS()
        lbfgs.run(wfn1, maxit=1000, thresh=1e-6,plev=0)
        Emin1 = wfn1.energy
        if self.check_minimum(wfn1):
        ...
        lbfgs.run(wfn2, maxit=1000, thresh=1e-6,plev=0)
        Emin2 = wfn2.energy
        if self.check_minimum(wfn2):
    ...
    def find_connections(self,stepsize=0.1,cycles=10):
        ...
        for icycle in range(cycles):
            ...
            for _, mtag in current_minima.items():
                ...
                if newts:
                    # Find the connection
                    (Imin1,Emin1), (Imin2,Emin2) = self.connect(wfnts)
