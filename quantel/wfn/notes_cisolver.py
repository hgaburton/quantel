...
class ArbitraryCI:
    ...
    """
    ## QUESTIONS: 
    ### where does the cispace object come from and what does it look like? 
...
class FCI(ArbitraryCI):
    ...
    def __init__(self, mo_ints, nelec, version=1):
        ...
        # Convert from quantel ints from pyscf ints - okay 
        if isinstance(mo_ints, PySCF_MO_Integrals):
            self.mo_ints = mo_ints.get_quantel_ints()
        else:
            self.mo_ints = mo_ints
        # Number of correlated orbitals - just number of linearly independent orbitals
        ...
        if(self.nalfa + self.nbeta > 2*self.nmo):
            ...
            # number of active is just number of electrons as this is a full CI 
