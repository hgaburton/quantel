...
def get_avas(integrals, exchange_matrix):
    ...
    eri = pymol.intor("int2e", aosym="s8")
    # These are the active exchange integrals in chemists order (pq|rs)
    pppp = ao2mo.restore('s1', pymol.ao2mo(cactive), ncas)
    K = np.einsum('pqqp->pq', pppp)
    order = optimise_order(K, exchange_matrix)
    cguess[:,ncore:ncore+ncas] = cactive[:,order]
...
def rohf_local_guess(integrals, exchange_matrix):
    ...
    #pppp = ao2mo.restore('s1', pymol.ao2mo(cactive), ncas)
    #K = np.einsum('pqqp->pq', pppp)
    # Optimise the ordering oopen-shell orbitals to minimize exchange energy
    order = optimise_order(K, exchange_matrix)
