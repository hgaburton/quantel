...
class EigenFollow:
    ...
    def __init__(self, **kwargs):
        ...
        self.control = dict()
        self.control["minstep"] = 0.001
        self.control["maxstep"] = 2 * np.pi
    ...
    def run(self, obj, thresh=1e-8, maxit=100, index=0, plev=1):
        ...
        for istep in range(maxit+1):
            ...
            if istep > 0 and plev > 0:
                print(" {: 5d} {: 16.10f}    {:8.2e}    {:8.2e}    {:10s}".format(istep, eref, step_length, conv, comment))
            elif plev > 0:
