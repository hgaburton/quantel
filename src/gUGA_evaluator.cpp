...
bool GUGAEval::contains(const std::vector<std::string>& vec, const std::string& val) const {
    return std::find(vec.begin(), vec.end(), val) != vec.end();
}

std::string GUGAEval::HeadsOrTails(const int &ind, const int &i, const int &j ) const { 
    if (ind == std::max(i,j)) { return "h";}
    else if ( ind == std::min(i,j)) {return "t"; }
    else { return "" ;}
}

int GUGAEval::get_Dind(const int &delta_b) const {
...
double GUGAEval::one_body_coupling(const Configuration &bra, const Configuration &ket, const Eph &Eph) const { 
...
//double GUGAEval::one_body_fragment(const Configuration &bra, const Configuration &ket, const int &level) const {  
double GUGAEval::one_body_fragment(const int &level, const int &d1, const int &d2, const int &b, const int &delta_b, const int &head, const int &tail, const int &RorL ) const { 
    double factor ; 
    if ( level == head) { 
    // Loop head 
...
double GUGAEval::two_body_coupling( const Configuration &bra, const Configuration &ket, const Epphh &Epqrs ) const { 
    // Check same number of electrons, orbitals and S 
    if ( (bra.m_nmo != ket.m_nmo) || (bra.m_nelec != ket.m_nelec) || (bra.m_totspin != ket.m_totspin)  ) { 
        std::cout << "Different N, n or S values" << std::endl ; 
        return 0.0 ; 
...
double GUGAEval::resolve_two_body_matrix_element( const Configuration &bra, const Configuration &ket, const Epphh &Epqrs, const std::vector<Configuration> &basis) const { 
    // Check same number of electrons, orbitals and S 
    if ( (bra.m_nmo != ket.m_nmo) || (bra.m_nelec != ket.m_nelec) || (bra.m_totspin != ket.m_totspin)  ) { 
        std::cout << "Different N, n or S values" << std::endl ; 
        return 0.0 ; 
